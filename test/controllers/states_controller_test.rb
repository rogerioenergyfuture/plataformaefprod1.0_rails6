...
class StatesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create state" do
    assert_difference('State.count') do
      post states_url, params: { state: { name: @state.name, region: @state.region, sigla: @state.sigla } }
    end
  ...
  test "should update state" do
    patch state_url(@state), params: { state: { name: @state.name, region: @state.region, sigla: @state.sigla } }
